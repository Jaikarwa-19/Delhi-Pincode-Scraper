...
def log_progress(df, listings):
    df.drop_duplicates(subset=["Name", "Pincode"], inplace=True)
    df.to_csv(PARTIAL_CSV, index=False)
    print(f"✅ Partial save: {len(df)} listings written to {PARTIAL_CSV}")
    print(f"↳ Latest listing: {listings[-1]['Name']} | {listings[-1].get('Pincode', '')}")
